...
export function SourceSelector({ options, value, onChange }: Props) {
  ...
  return (
    <section className="rounded-3xl border border-line bg-panel/95 p-5 shadow-panel">
      <SectionHeader
        title="Source view"
        description="Choose the current record source: live runtime state or a canonical example fixture."
      />

      <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(220px,1fr)_minmax(140px,0.7fr)_minmax(140px,0.7fr)_minmax(320px,1.2fr)] xl:items-end">
        <MetricCard label="Current source" value={selected?.label ?? value} emphasis="strong" />
        ...
        <label className="min-w-0 rounded-2xl border border-line bg-ink/70 px-4 py-3">
          <div className="text-[11px] uppercase tracking-[0.18em] text-muted">Record source</div>
          <select
            value={value}
          ...
          >
            {options.map((option) => (
              <option key={option.id} value={option.id}>
                {option.kind === 'example' ? `🟣 Example · ${option.label}` : option.label}
              </option>
            ))}
          </select>
          {selected?.category ? <div className="mt-2 text-xs text-muted">{selected.category}</div> : null}
        </label>
      </div>
    </section>
  );
